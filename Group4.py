    ...
    for p in range(1, last_page_index + 1):
        ...
        for container in containers:
            try:
                ...
                price = container.find('span', class_='listing-card__price__ value').text.strip().replace("\u202f", "").replace("F Cfa", "")
                image = container.find('div', class_='listing-card__image__inner').img['src']
                dic = {
                    'condition': condition,
...
if choices == 'Scrape data using BeautifulSoup':
    ...
    motocycle = pd.read_csv('cars.csv')
    ...
    df1 = pd.read_csv('vehicule.csv')
    df2 = pd.read_csv('moto.csv')
    df3 = pd.read_csv('pièces.csv')
